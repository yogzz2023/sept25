    ...
    for group in measurement_groups:
        ...
        for track_id, track in enumerate(tracks):
            ...
            if range_satisfied:
                ...
                track['current_state'] = state_map[track_id] # Update the track's current state
                assigned = True
                break
        ...
        if not assigned:
            ...
            tracks.append({
                ...
                'Pp': np.eye(6),
                'Sp': np.zeros((6, 1)),
                'current_state': progression_states[0] # Set initial state
            })
...
def main():
            ...
            for track_id, track in enumerate(tracks):
                ...
                if current_state == 'Poss1' or current_state == 'Tentative1':
                    ...
                    if distance < range_threshold:
                        ...
                        track['measurements'].append((group[0], new_state)) # Add measurement with new state
                        break
            ...
            if not assigned:
                ...
                if free_track_idx is not None:
                    ...
                    tracks.append({
                        ...
                        'Sf': np.zeros((6, 1)),  # Filter state vector
                        'Pf': np.eye(6),  # Filter state covariance matrix
                        'Pp': np.eye(6),  # Predicted state covariance matrix
                        'Sp': np.zeros((6, 1)),  # Predicted state vector
                        'current_state': 'Poss1' # Set initial state
                    })
    ...
    with open('updated_filter_states.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Track ID', 'Current State', 'Measurement History', 'Final Sf', 'Final Pf', 'Final Pp'])
        for track in tracks:
            measurements_str = '; '.join([f"({m[0]}, {m[1]}, {m[2]}, {m[3]}, {s})" for m, s in track['measurements']])
            writer.writerow([track['track_id'], track['current_state'], measurements_str, track['Sf'].flatten(), track['Pf'].flatten(), track['Pp'].flatten()])
