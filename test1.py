...
def main():
    ...
    with open('updated_filter_states.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Track ID', 'State', 'Measurements', 'Sf', 'Pf', 'Pp'])
        for track in tracks:
            measurements_str = '; '.join([f"({m[0]}, {m[1]}, {m[2]}, {m[3]})" for m in track['measurements'] if len(m)>=4])
            writer.writerow([track['track_id'], track['state'], measurements_str, track['Sf'].flatten(), track['Pf'].flatten(), track['Pp'].flatten()])
    
    print("Track Summary:")
    for track in tracks:
        print(f"Track ID: {track['track_id']}, State: {track['state']}, Measurements: {track['measurements']}, Sf: {track['Sf'].flatten()}, Pf: {track['Pf'].flatten()}, Pp: {track['Pp'].flatten()}")
