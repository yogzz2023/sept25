...
def main():
    ...
    for group_idx, group in enumerate(measurement_groups):
        ...
        for i in range(len(tracks) - 1, -1, -1):  # Iterate in reverse to safely remove
            track = tracks[i]
    ...
    # Print Track ID Status
    print("\nTrack ID Status:")
    for track in track_id_list:
        print(f"Track ID: {track['id']}, Status: {track['state']}")
